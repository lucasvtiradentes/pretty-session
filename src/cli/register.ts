...
export function registerProgram(program: CaporalProgram, commands: readonly CommandDefinition[]): void {
	for (const command of commands) {
		if (command.kind === 'command') {
			...
			for (const subcommand of command.subcommands) registerSubCommand(program, command.name, subcommand)
			continue
		}
...
function registerParentCommand(program: CaporalProgram, definition: ParentCommandDefinition): CaporalCommand {
	...
	command.action(({ program }) => {
		if (definition.action) return definition.action({ program })

		console.log(`Usage: ${program.getBin() || CLI_NAME} ${definition.name} <command>`)
		console.log('')
...
function registerSubCommand(
	...
	command.strict(definition.strict ?? false)

	command.action(({ args, options, program }) =>
		definition.action({
...
function applyFlags(command: CaporalCommand, flags: readonly CommandFlag[] | undefined): void {
	...
	for (const flag of flags) {
		...
		if (flag.alias) flagString = `${flag.alias}, ${flagString}`
		if (flag.type === CommandFlagType.String) flagString += ' <value>'
