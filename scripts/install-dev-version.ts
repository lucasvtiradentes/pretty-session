...
exec ${shellQuote(join(rootDir, 'node_modules', '.bin', 'tsx'))} --conditions=development ${shellQuote(join(rootDir, 'src', 'bin.ts'))} "$@"
`
}
...
"${join(rootDir, 'node_modules', '.bin', 'tsx.cmd')}" --conditions=development "${join(rootDir, 'src', 'bin.ts')}" %*
`
}
